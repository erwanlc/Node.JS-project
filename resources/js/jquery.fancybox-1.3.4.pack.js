(function (B) {
  var L, T, Q, M, d, m, J, A, O, z, C = 0, H = {}, j = [], e = 0, G = {}, y = [], f = null, o = new Image(), i = /\.(jpg|gif|png|bmp|jpeg)(.*)?$/i, k = /[^\.]\.(swf)\s*$/i, p, N = 1, h = 0, t = "", b, c, P = false, s = B.extend(B("<div/>")[0], {prop: 0}), S = navigator.userAgent.match(/msie/i) && navigator.userAgent.match(/6/) && !window.XMLHttpRequest, r = function () {
    T.hide();
    ...
    if (G.type == "iframe") {
      B('<iframe id="fancybox-frame" name="fancybox-frame' + new Date().getTime() + '" frameborder="0" hspace="0" ' + (navigator.userAgent.match(/msie/i) ? 'allowtransparency="true""' : "") + ' scrolling="' + H.scrolling + '" src="' + G.href + '"></iframe>').appendTo(m);
    }
    M.show();
    P = false;
    B.fancybox.center();
  ...
  B.fn.fancybox = function (V) {
    ...
    B(this).data("fancybox", B.extend({}, V, (B.metadata ? B(this).metadata() : {}))).unbind("click.fb").bind("click.fb", function (X) {
      ...
      j = [];
      C = 0;
      var W = B(this).attr("data-group") || "";
      if (!W || W == "" || W === "nofollow") {
        j.push(this);
      } else {
        j = B("a[data-group=" + W + "], area[data-group=" + W + "], img[data-group=" + W + "]");
        C = j.index(this);
      }
      w();
      return;
