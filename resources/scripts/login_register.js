...
jQuery('.form-register').submit(function(e) {
  ...
  if (password_ === jQuery('#register-password-confirm').val() && email_ && password_) {
    var data = {
      email: email_,
      password: password_
    };
    tools.request('/register', data, function(msg) {
      ...
      toastr.error('Something went wrong ' + err.toString);
    });
  } else {
    toastr.warning('Passwords doesn\'t match or no email', 'Something is not correct');
  }
});
...
jQuery('.btn-logout').on('click', function() {
  tools.request('/log-out', {}, null, function() {
    toastr.success('Log out successful', 'Goodbye !');
    tools.redirect('/');
  });
});
